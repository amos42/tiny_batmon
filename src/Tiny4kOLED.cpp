...
void SSD1306Device::sendDataStart(void) {
	::ssd1306_send_data_start();
}

void SSD1306Device::sendDataByte(uint8_t byte) {
	::ssd1306_send_data_byte(byte);
}

void SSD1306Device::sendStop(void) {
	::ssd1306_send_stop();
}
...
size_t SSD1306Device::write(byte c) {
	...
	if((uint16_t)c < oledFont->first || (uint16_t)c >= (oledFont->first + oledFont->last)) {
		offset = 0xffff;
	} else {
		offset = ((uint16_t)c - oledFont->first) * w * h;
	}
	...
	do
	{
		...
		for (uint8_t i = 0; i < w; i++) {
			uint8_t v;
			if(offset != 0xffff)
				v = pgm_read_byte(&(oledFont->bitmap[offset++]));
			else
				v = 0x00;
			ssd1306_send_data_byte(v);
		}
		ssd1306_send_stop();
		if (h == 1) {
			oledX+=w;
